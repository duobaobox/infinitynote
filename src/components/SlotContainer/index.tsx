...
export const SlotContainer: React.FC<SlotContainerProps> = ({
  ...
  return (
    <div className={`${styles.slotContainer} ${className}`} data-slot-container>
      {/* 插槽列表 */}
      <div className={styles.slotsList} id="slots-list">
        {connectedNotes.length === 0 ? (
          <div className={styles.emptySlots}>
            <div className={styles.emptySlotCircle}>
              <span className={styles.emptySlotIcon}>○</span>
            </div>
            <div className={styles.emptyTextContainer}>
              <span className={styles.emptyText}>暂无连接的便签</span>
              <span className={styles.emptyHint}>
                悬停便签并点击左下角{" "}
                <span className={styles.connectionDotDemo}>●</span>{" "}
                连接点来建立连接
              </span>
            </div>
          </div>
